...
function VectorTileClipper(feature) {
    ...
    this._isLine = feature.type !== 3; // consider points like line (ignore evertyhing outside)
}
...
VectorTileClipper.prototype.result = function() {
    ...
    /*// need to transform coordinates to go back to the expected 4096 extent
     ...
     // transform
     pt.x = (pt.x - (this.xmin + this.margin)) * this.finalRatio;
     pt.y = (pt.y - (this.ymin + this.margin)) * this.finalRatio;
     }
     }
...
VectorTileClipper.prototype._intersect = function(inPt, outPt) {
    ...
    else
    {
        ...
        var xRatio = (x - inPt.x) / (outPt.x - inPt.x);
        var yRatio = (y - inPt.y) / (outPt.y - inPt.y);
        if (xRatio < yRatio)
        {
...
VectorTileFeature.prototype.loadClippedGeometry = function() {
    if (!this.dz && !this.xPos && !this.yPos)
        return [];
